...
function rank(){const risk=document.getElementById('risk').value,useNlp=document.getElementById('useNlp').checked,useCv=document.getElementById('useCv').checked,rr=document.getElementById('rerank').checked;const allow=risk==='low'?['low']:risk==='medium'?['low','medium']:['low','medium','high'];let out=assets.filter(x=>allow.includes(x.risk)).map(x=>({...x,value:x.base+(useNlp?x.nlp:0)+(useCv?x.cv:0)})).sort((a,b)=>b.value-a.value);if(rr){out=out.map((x,i)=>({...x,value:x.value+(x.risk==='low'?.025:0)-(i>7?.01:0)})).sort((a,b)=>b.value-a.value);}renderScores('ranking',out.slice(0,10));document.getElementById('recConclusion').textContent=`Допущено ${out.length} из ${assets.length} кандидатов. Нарушений после фильтра: 0. ${useNlp&&useCv?'NLP и CV влияют на порядок и основания.':'Отключение модальности используется как абляция.'}`;}
