...
function analyzeVision(){drawMarket(true);const first=series.slice(0,6).reduce((a,x)=>a+x.close,0)/6;const last=series.slice(-6).reduce((a,x)=>a+x.close,0)/6;const delta=(last-first)/first;const trend=delta>.04?'uptrend':delta<-.04?'downtrend':'sideways';const noise=Number(document.getElementById('noise').value);const confidence=Math.max(.61,.96-noise*.0025-(document.getElementById('watermark').checked?.04:0));document.getElementById('trendOut').textContent=trend;document.getElementById('diceOut').textContent=(.91-noise*.0012).toFixed(3).replace('.',',');document.getElementById('cvConfidence').textContent=confidence.toFixed(2).replace('.',',');document.getElementById('visionStatus').textContent='plot area обнаружена';document.getElementById('cvActionConclusion').textContent=confidence<.75?'Качество изображения снижает уверенность. Система использует text-only fallback.':'Визуальный признак пригоден для fusion; версия и время расчета сохраняются.';}
document.getElementById('generateChart').addEventListener('click',createSeries);document.getElementById('runVision').addEventListener('click',analyzeVision);document.getElementById('watermark').addEventListener('change',()=>drawMarket(false));document.getElementById('noise').addEventListener('input',()=>drawMarket(false));
